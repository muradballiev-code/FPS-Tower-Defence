...
namespace GameDevHQ.FileBase.Plugins.FPS_Character_Controller
{
    ...
    public class FPS_Controller : MonoBehaviour
    {
        ...
        [SerializeField]
        private bool _isZoom = false; //bool to zoom sniper scope
        private float _coolShoot = 0.5f; //CoolDown system to shot
        private float _coolTimer = 0.3f; //timer to shot 
        [SerializeField]
        private AudioClip _shootSound; //sound for shot
        [SerializeField]
        private AudioClip _zoomSound; //sound for scope zoom
        [SerializeField]
        private AudioSource _audioSource; //audiosouce component

        [SerializeField]
        private GameObject _bulletPrefab; //bullet prefab to instantiate
        [SerializeField]
        private GameObject _bulletParentContainer; //container to keep buller prefab
        [SerializeField]
        private Transform _firePoint; //bullet spawn point
        ...
        private void Start()
        {
            ...
            _initialCameraPos = _fpsCamera.transform.localPosition; //point of bullet exit
        ...
        private void Update()
        {
            ...
            //Click Left Mouse Button to shot
            if (Input.GetMouseButtonDown(0) && Time.time > _coolShoot)
            {
                //Set new timer for next shoting
                _coolShoot = Time.time + _coolTimer;
                //Call Shoot method
                PlayerShoot();
            }
        ...
        public void PlayerShoot()
        {
            ...
            //Set LayerMask to the Ray
            if (Physics.Raycast(position, out hit, Mathf.Infinity, LayerMask.GetMask("Enemy", "Barrier", "Barrel")))
            {
                //If hit Someone
        ...
        public void Zoom()
        {
            ...
            {
                _isZoom = true;
                _fpsCamera.fieldOfView = 20f; //When right mouse button click set Zoom to 20
                AudioSource.PlayClipAtPoint(_zoomSound, _fpsCamera.transform.position); //Zoom sound when right Mousebutton was pressed
            }
            else if (Input.GetMouseButtonUp(1) && _isZoom == true)
            {
                _isZoom = false;
                _fpsCamera.fieldOfView = 55f; //Else set Zoom to 55f
                AudioSource.PlayClipAtPoint(_zoomSound, _fpsCamera.transform.position); //Zoom sound when right Mousebutton was released
            }
        }
